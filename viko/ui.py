    ...
    def paintEvent(self, _):
        ...
        if self._face_px:
            ...
            p.drawText(QRectF(cx - 80, cy - 14, 160, 28),
                       Qt.AlignmentFlag.AlignCenter, "VIKO")
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith("viko:"):   self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for VIKO", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure VIKO before first boot.", 11, color=C.PRI_DIM))
        layout.addSpacing(6)
        ...
        self._or_input.setFont(QFont("Courier New", 11))
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        super().__init__()
        self.setWindowTitle("VIKO - B.0.1.0")
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("B.0.1.0", C.PRI_DIM))
        lay.addStretch()
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("VIKO  ·  B.0.1.0  ·  CLASSIFIED"))
        lay.addStretch()

        self._panel_btn = QPushButton()
        self._panel_btn.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
        self._panel_btn.setFixedHeight(16)
        self._panel_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._panel_btn.clicked.connect(self._toggle_right_panel)
        self._style_panel_btn()
        lay.addWidget(self._panel_btn)

        return w

    def _style_panel_btn(self):
        visible = self._right_panel.isVisible()
        self._panel_btn.setText("◧  PANEL  ▶" if visible else "◨  PANEL  ◀")
        col = C.PRI if visible else C.TEXT_DIM
    ...
    def _on_file_selected(self, path: str):
        ...
        self._file_hint.setText(f"{icon}  {p.name}  ·  {size}  ·  Tell VIKO what to do with it")
    ...
    def _check_config(self) -> bool:
        try:
            from viko.config import is_configured
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str):
        from viko.config import save_keys
        save_keys(gemini_api_key=key, openrouter_api_key=or_key, os_system=os_name)
        self._ready = True
        if self._overlay:
            self._overlay.hide()
            self._overlay = None
        self._apply_state("LISTENING")
        self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. Viko online.")
