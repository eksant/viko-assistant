...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    try:
        exec(compile(code, "<viko_desktop>", "exec"), sandbox)
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            print(f"[Desktop] Asking Viko: {actual_task}")
            if player:
                player.write_log("[Desktop] Generating action...")

            code = _ask_viko_for_desktop_action(actual_task)
            return _execute_generated_code(code, player=player)

        else:
            if action:
                code = _ask_viko_for_desktop_action(action)
                return _execute_generated_code(code, player=player)
