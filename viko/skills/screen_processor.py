...
def _get_camera_index() -> int:
    ...
    # Persist to os.environ and .env
    os.environ["CAMERA_INDEX"] = str(best_index)
    try:
        lines = []
        if ENV_PATH.exists():
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith("CAMERA_INDEX="):
                lines[i] = f"CAMERA_INDEX={best_index}"
                updated = True
                break
        if not updated:
            lines.append(f"CAMERA_INDEX={best_index}")
        ENV_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
        print(f"[Camera] 💾 Camera index {best_index} saved to .env.")
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript_buf and self._player:
                        ...
                        if full:
                            self._player.write_log(f"Viko: {full}")
