"""
file_processor.py — Viko Universal File Processor
...
def _generate(contents, model: str = "gemini-2.5-flash"):
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("describe", "ocr", "analyze", "read", "extract_text"):
        try:
            ...
            response = _generate([
                prompt,
                _gt.Part.from_bytes(data=buf.getvalue(), mime_type="image/png"),
            ])
    ...
    if action == "transcribe":
        try:
            ...
            response = _generate([
                ...
                _gt.Part.from_bytes(data=content, mime_type=mime),
            ])
            result = response.text.strip()
